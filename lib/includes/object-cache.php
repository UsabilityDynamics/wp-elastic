...
class WP_Object_Cache {
   ...
   * @var bool
   */
  private $wp_elastic_connected = false;
   ...
   * @var array
   */
  public $no_wp_elastic_groups = array( 'comment', 'counts' );
  ...
  public function __construct() {
    ...
    if ( defined( 'WP_ELASTIC_BACKEND_HOST' ) && WP_ELASTIC_BACKEND_HOST ) {
      $wp_elastic['host'] = WP_ELASTIC_BACKEND_HOST;
    }
    if ( defined( 'WP_ELASTIC_BACKEND_PORT' ) && WP_ELASTIC_BACKEND_PORT ) {
      $wp_elastic['port'] = WP_ELASTIC_BACKEND_PORT;
    }
    if ( defined( 'WP_ELASTIC_BACKEND_AUTH' ) && WP_ELASTIC_BACKEND_AUTH ) {
      $wp_elastic['auth'] = WP_ELASTIC_BACKEND_AUTH;
    }
    if ( defined( 'WP_ELASTIC_BACKEND_INDEX' ) && WP_ELASTIC_BACKEND_INDEX ) {
      $wp_elastic['index'] = WP_ELASTIC_BACKEND_INDEX;
    }

    // Use Redis PECL library if available, otherwise default to bundled Predis library
    if ( class_exists( 'Redis' ) ) {
      try {
        $this->wp_elastic = new Redis();
        $this->wp_elastic->connect( $wp_elastic['host'], $wp_elastic['port'] );

        if ( isset( $wp_elastic['auth'] ) ) {
          $this->wp_elastic->auth( $wp_elastic['auth'] );
        }

        if ( isset( $wp_elastic['database'] ) ) {
          $this->wp_elastic->select( $wp_elastic['database'] );
        }

        $this->wp_elastic_connected = true;
      } catch ( RedisException $e ) {
        $this->wp_elastic_connected = false;
      }
    } else {
      try {
        require_once 'elastic/autoload.php';
        $this->wp_elastic = new Predis\Client( $wp_elastic );
    ...
    if ( ! $this->wp_elastic_connected ) {
      $this->no_wp_elastic_groups = array_unique( array_merge( $this->no_wp_elastic_groups, $this->global_groups ) );
    }
   ...
   * @return bool
   */
  protected function can_wp_elastic() {
    return $this->wp_elastic_connected;
  }
  ...
  protected function add_or_replace( $add, $key, $value, $group = 'default', $expiration = 0 ) {
    ...
    if ( in_array( $group, $this->no_wp_elastic_groups ) || ! $this->can_wp_elastic() ) {
    ...
    // Check if conditions are right to continue
    if (
      ( $add   &&   $this->wp_elastic->exists( $derived_key ) ) ||
      ( ! $add && ! $this->wp_elastic->exists( $derived_key ) )
    ) {
      return false;
    }
    ...
    if ( $expiration ) {
      $result = $this->parse_elastic_response( $this->wp_elastic->setex( $derived_key, $expiration, $this->prepare_value_for_wp_elastic( $value ) ) );
    } else {
      $result = $this->parse_elastic_response( $this->wp_elastic->set( $derived_key, $this->prepare_value_for_wp_elastic( $value ) ) );
    }
  ...
  public function delete( $key, $group = 'default' ) {
    ...
    // Remove from no_wp_elastic_groups array
    if ( in_array( $group, $this->no_wp_elastic_groups ) || ! $this->can_wp_elastic() ) {
    ...
    $result = $this->parse_elastic_response( $this->wp_elastic->del( $derived_key ) );
  ...
  public function flush( $delay = 0 ) {
    ...
    if ( $this->can_wp_elastic() ) {
      $result = $this->parse_elastic_response( $this->wp_elastic->flushdb() );
    }
  ...
  public function get( $key, $group = 'default' ) {
    ...
    if ( $this->wp_elastic->exists( $derived_key ) ) {
      $this->cache_hits++;
      $value = $this->restore_value_from_wp_elastic( $this->wp_elastic->get( $derived_key ) );
    } else {
      $this->cache_misses;
      return false;
    }
   ...
   *
   * @param   array                           $groups  Array of groups and keys to retrieve
   * @uses    this::filter_wp_elastic_get_multi()
  ...
  public function get_multi( $groups ) {
    ...
    foreach ( $groups as $group => $keys ) {
      if ( in_array( $group, $this->no_wp_elastic_groups ) || ! $this->can_wp_elastic() ) {
        ...
        $group_cache = $this->wp_elastic->mget( $derived_keys );
        ...
        $group_cache = array_map( array( $this, 'restore_value_from_wp_elastic' ), $group_cache );

        // Redis returns null for values not found in cache, but expected return value is false in this instance
        $group_cache = array_map( array( $this, 'filter_wp_elastic_get_multi' ), $group_cache );
  ...
  public function increment( $key, $offset = 1, $group = 'default' ) {
    ...
    // Save to Redis
    $result = $this->parse_elastic_response( $this->wp_elastic->incrBy( $derived_key, $offset ) );

    $this->add_to_internal_cache( $derived_key, (int) $this->wp_elastic->get( $derived_key ) );
  ...
  public function decrement( $key, $offset = 1, $group = 'default' ) {
    ...
    // Save to Redis
    $result = $this->parse_elastic_response( $this->wp_elastic->decrBy( $derived_key, $offset ) );
  ...
  public function stats() {
    ...
    <strong><?php _e( 'Cache Hits:', 'wordpress-redis-backend' ); ?></strong> <?php echo number_format_i18n( $this->cache_hits ); ?><br />
			...
			<strong><?php _e( 'Using Redis?', 'wordpress-redis-backend' ); ?></strong> <?php echo $this->can_wp_elastic() ? __( 'yes', 'wordpress-redis-backend' ) : __( 'no', 'wordpress-redis-backend' ); ?><br />
    </p>
    <p>&nbsp;</p>
   ...
   * @return string
   */
  protected function prepare_value_for_wp_elastic( $value ) {
   ...
   * @return mixed
   */
  protected function restore_value_from_wp_elastic( $value ) {
   ...
   */
  protected function filter_wp_elastic_get_multi( $value ) {
    if ( is_null( $value ) ) {
      $value = false;
    }
   ...
   * @return mixed
   */
  protected function parse_elastic_response( $response ) {
    if ( is_bool( $response ) ) {
      return $response;
    }
  ...
  public function add_global_groups( $groups ) {
    ...
    if ( $this->can_wp_elastic() ) {
      ...
      $this->no_wp_elastic_groups = array_unique( array_merge( $this->no_wp_elastic_groups, $groups ) );
    }
  }
