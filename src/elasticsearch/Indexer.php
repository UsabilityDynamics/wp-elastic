...
namespace elasticsearch {
  ...
  class Indexer {
    ...
    static function per_page() {
      return Config::apply_filters( 'indexer_per_page', 20 );
    }
    ...
    static function reindex( $page = 1 ) {
      ...
      foreach( (array) $posts as $post ) {
        $response = self::addOrUpdate( $post )->getData();
    ...
    static function delete( $post ) {
      ...
      $_options = (array) Config::option( 'server_options' );

      if( $_options[ 'use_bulk' ] ) {

        //$client->deleteIds( array( $post->ID ), Config::option('server_index'), $post->post_type );

        // Create new Bulk client/instance thing.
        $bulk = new \Elastica\Bulk( $index->getClient() );

        // Include Index and Type in PUT url.
        if( $_options[ 'use_full_path' ] ) {
          $bulk->setIndex( Config::option( 'server_index' ) );
          $bulk->setType( $post->post_type );
        }

        // We only have one action.
        $action = new \Elastica\Bulk\Action( \Elastica\Bulk\Action::OP_TYPE_DELETE );
        $action->setIndex( Config::option( 'server_index' ) );
        $action->setType( $post->post_type );
        $action->setId( $post->ID );
      ...
      try {
        return $type->deleteById( $post->ID );
    ...
    static function addOrUpdate( $post ) {
      ...
      if( $_options[ 'use_bulk' ] ) {
        ...
        // We only have one action.
        $action = new \Elastica\Bulk\Action( \Elastica\Bulk\Action::OP_TYPE_INDEX );
        $action->setIndex( Config::option( 'server_index' ) );
        $action->setType( $post->post_type );
        $action->setId( $post->ID );
      ...
      $response = $type->addDocument( new \Elastica\Document( $post->ID, $data ) );
    ...
    static function _client( $write = false ) {
      ...
      $_server_url = trailingslashit( Config::option( 'server_url' ) );

      if( Config::option( 'server_auth' ) ) {
        $_server_url = Config::option( 'server_auth' ) . '@' . $_server_url;
      }
